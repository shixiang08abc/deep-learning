...
class Word2Vec(object):
  ...
  def getTermWeightBactchDate(self, fin):
    batch_cursor = 0
    ...
    target_list = np.zeros([self.batch_size, self.num_steps+self.target_delay], dtype=np.int32)
    ...
    while True:
      ...
      for i in range(len(terms)):
        try:
          ...
          target_list[batch_cursor, i+self.target_delay] = int(labels[i])
