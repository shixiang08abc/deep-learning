...
class Word2Vec(object):
  def __init__(self,config):
    ...
    with tf.device("cpu:0"):
      ...
      self.id2parseembedding = tf.get_variable("parse_embedding", [self.parse_size+1,self.embedding_dim,self.parse_dim], dtype=tf.float32,trainable=True)
  ...
  def loadParseEmbedding(self,session,filepath):
    sys.stderr.write("\nloading parse embeddings form tensor file...")
    saver = tf.train.Saver({'parse_embedding':self.id2parseembedding})
    saver.restore(session,filepath)
    sys.stderr.write("\nloading parse embeddings finished !!!")
  ...
  def getImportanceBatchData(self, fin):
    ...
    parse_head_list = np.zeros([self.batch_size, self.num_steps+self.target_delay], dtype=np.int32)
    parse_deprel_list = np.zeros([self.batch_size, self.num_steps+self.target_delay], dtype=np.int32)
    ...
    while True:
      ...
      if line=="":
        if batch_cursor==0:
          return False,data_list,tag_list,parse_head_list,parse_deprel_list,target_list,length_list,frame_weight
        else:
          return True,data_list,tag_list,parse_head_list,parse_deprel_list,target_list,length_list,frame_weight
      ...
      heads = tokens[3].strip().split(" ")
      deprels = tokens[4].strip().split(" ")
      if len(terms)!=len(labels) or len(terms)!=len(tags) or len(terms)!=len(heads) or len(terms)!=len(deprels):
      ...
      for i in range(len(terms)):
        try:
          ...
          parse_deprel_list[batch_cursor, i] = int(deprels[i].strip())
          if deprels[i].strip()=="1" and heads[i].strip()=="0":
            parse_head_list[batch_cursor, i] = self.vocab_size - 1
          else:
            hidx = int(heads[i])
            parse_head_list[batch_cursor, i] = self.getWid(terms[hidx].strip())
