...
void QrnnDecode::init_fast_tanh()
{
	tanh_cache_ = new float[tanh_size_];

	float step = 16.0f / (tanh_size_-1);
	for (size_t i = 0; i < tanh_size_; i++)
	{
		tanh_cache_[i] = tanh(-8.0f+i*step);
	}
}



float QrnnDecode::fast_tanh(float x)
{
	if (x<-8.0f)
		return -1.0f;
	else if(x>8.0f)
	...
	{
		return tanh_cache_[static_cast<size_t>((x+8.0f)*1000000.0f)];
	}
}
...
bool QrnnDecode::ReadModelBin(const std::string& model_path)
{
	...
	if(str == "embedding_dict")
	{
		...
		ifs.read((char*)&feat, sizeof(int));
		size_t column = feat;
		if(column == cell_size_*filter_width_*3+1)
		{
			for(size_t i=0; i<word_size_; i++)
			{
				str.clear();
				...
				float* embedding = new float[cell_size_*filter_width_*3];
				ifs.read((char*)embedding, cell_size_*filter_width_*3*sizeof(float));
...
void QrnnDecode::Propagate(const float* mat_input, size_t input_row, size_t input_col, float* mat_output, size_t output_row, size_t output_col)
{
	...
	float input_zfo_buf[input_row*input_col];

	memset(cell_buf, 0.0, (input_row+1)*cell_size_*sizeof(float));
	memcpy(input_zfo_buf, mat_input, input_row*input_col*sizeof(float));
	...
	for(size_t i=0; i<input_row; i++)
	{
		for(size_t j=0; j<cell_size_; j++)
		{
			...
			//input_zfo_buf[i*3*cell_size_+j] = fast_tanh(input_zfo_buf[i*3*cell_size_+j]);
			//input_zfo_buf[i*3*cell_size_+cell_size_+j] = 0.5*(fast_tanh(input_zfo_buf[i*3*cell_size_+cell_size_+j]*0.5) + 1);
			//input_zfo_buf[i*3*cell_size_+2*cell_size_+j] = 0.5*(fast_tanh(input_zfo_buf[i*3*cell_size_+2*cell_size_+j]*0.5) + 1);
		}
	}
	gettimeofday(&tv2, NULL);
	size_t span = (tv2.tv_sec-tv1.tv_sec)*1000000+(tv2.tv_usec-tv1.tv_usec);
	std::cout << "CPU time1 used: " << span << " us" << std::endl;
	...
	span = (tv3.tv_sec-tv1.tv_sec)*1000000+(tv3.tv_usec-tv1.tv_usec);
	std::cout << "CPU time2 used: " << span << " us" << std::endl;

	//std::cout << "cell_buf" << std::endl;
	//for(size_t i=0; i<input_row+1; i++)
	//{
	//	for(size_t j=0; j<cell_size_; j++)
	...
	//cala mat_output
	//cell_buf[(input_row+1)*cell_size_]
	//mat_affine_w_[target_num_*cell_size_]
	//mat_input[input_row*target_num_]
	for(size_t i=0; i<input_row; i++)
	{
	...
	span = (tv4.tv_sec-tv1.tv_sec)*1000000+(tv4.tv_usec-tv1.tv_usec);
	std::cout << "CPU time3 used: " << span << " us" << std::endl;
...
void QrnnDecode::getImportant(std::vector<std::string>& words, std::vector<float>& scores)
{
	...
	{
		memcpy(mat_input+i*3*cell_size_, vec_conv_bias_zfo_, 3*cell_size_*sizeof(float));

		for(size_t j=0; j<filter_width_; j++)
		{
			int sidx = i - (filter_width_-1-j);		//cnn start index
			if(sidx >= 0)
			{
				float* embedding = word_embedding_[getWordId(words[sidx])];
				for(size_t k=0; k<3*cell_size_; k++)
				{
					mat_input[i*input_col + k] += embedding[j*3*cell_size_ + k];
				}
			}
		}
	}*/
	...
	for(size_t i=0; i<words.size(); i++)
	{
		...
		for(size_t j=0; j<filter_width_; j++)
		{
			...
			{
				float* embedding = word_embedding_[getWordId(words[sidx])];
				cblas_saxpy(3*cell_size_, 1.0, embedding+j*3*cell_size_, 1, mat_input+i*input_col, 1);
			}
		}
	}
...
void QrnnDecode::printQrnnParaMeter()
{
	...
	for(size_t i=0; i<word_size_; i++)
	{
		...
		for(size_t j=0; j<cell_size_*filter_width_*3; j++)
...
int main(void)
{
	...
	if(!model.ReadModelBin("/data/shixiang/qrnn_imp/qrnn.model.dict.bin"))
	{
